...
# if "report_saved_status" not in st.session_state:
#     st.session_state.report_saved_status = False
...
#     "Focus Area",
#     options=["mixed", "technical", "behavioral", "case"]
# )

# start_button = st.sidebar.button("🚀 Start / Restart Interview", type="primary")

# if start_button:
#     full_bg = f"Candidate Name: {candidate_name}. {candidate_background}"
#     profile = CandidateProfile(
#         target_role=target_role,
#         candidate_background=full_bg,
...
#                 st.session_state.report_saved_status = True
#                 st.rerun()
...
#             )
#         with col_b:
#             if st.button("💾 Force Save to Sidebar"):
#                 timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#                 clean_name = candidate_name.replace(" ", "_")
#                 clean_role = target_role.replace(" ", "_")
#                 save_filename = f"{clean_name}_{clean_role}_{timestamp}.md"
#                 with open(REPORTS_DIR / save_filename, "w", encoding="utf-8") as f:
#                     f.write(st.session_state.coaching_report)
